...
Surgical compression still fires on the full staged set. A 26-file initial commit with 132,854 characters of diff compresses to 6,020 characters. That is 95% reduction, ~33.2k tokens down to ~1.5k, before the model sees anything.

The result:

```
feat(convit): generate conventional commits with ai

- scans codebase diff, extracts changes via AST-lite
- uses openai model to produce commit messages
- supports config file .convitrc.json for scopes and rules
- CLI tool 'convit' with init and dry-run options
- includes security checks for sensitive data
```
