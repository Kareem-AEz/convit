# convit — config architecture

How to architect a `.convitrc.json` from a project's structure, from first
principles. Read this when the task is to **generate or refine** a convit
config — the SKILL.md "Generate `.convitrc.json`" step points here. Follow the
five phases in order: Structural Analysis → Weighting → Synthesis → Ask → Write.

The config should capture **intent**, not just file paths. Never include a
`provider` block — credentials (`CONVIT_URL`, `CONVIT_KEY`, `CONVIT_MODEL`)
belong in `.env` only, and the config file is safe to commit. For the semantics
of every field, see [config-reference.md](config-reference.md).
...
Run it from this skill's directory (the path above is relative to `references/`);
if the skill is installed elsewhere, point at its `scripts/ensure-convit-env.mjs`.
From the project root, omit the path argument. The script checks `.env.local`
first, then `.env`, and appends a dev-only block (with a box comment) noting that
these vars should not be committed or uploaded.
